...
const initItems = [
  {id:1,title:"빨간 드레스 (2벌)",category:["소품","의상"],itemName:"빨간 드레스",price:0,photos:[],emoji:"🎭",desc:"공연 종료 후 남은 빨간 드레스 2벌. 사이즈 S/M. 세탁 완료.",seller:"극단 파도",sellerId:"user1",si:"파",likes:12,region:"서울 종로구",contact:"010-****-1234",safeNum:false,tradePlace:"대학로 마로니에공원 앞",status:"selling"},
  {id:2,title:"소형 스팟 조명 x3",category:["장비"],itemName:"스팟 조명",price:15000,photos:[],emoji:"💡",desc:"전시 철수 후 남은 LED 스팟 조명 3개. 상태 양호.",seller:"아트스페이스 을지",sellerId:"user2",si:"을",likes:8,region:"서울 중구",contact:"안심번호",safeNum:true,tradePlace:"을지로 3가역 2번 출구",status:"reserved"},
  {id:3,title:"나무 의자 세트 (6개)",category:["세트"],itemName:"나무 의자",price:0,photos:[],emoji:"🪑",desc:"무대 소품용 나무 의자 6개. 픽업 가능.",seller:"극단 숲",sellerId:"user3",si:"숲",likes:21,region:"서울 종로구",contact:"010-****-9012",safeNum:false,tradePlace:"대학로 소극장 앞",status:"done"},
  {id:4,title:"한복 치마 (연두색)",category:["의상"],itemName:"한복 치마",price:8000,photos:[],emoji:"👗",desc:"한복 작업 후 남은 치마 1점. 허리 60cm.",seller:"무용단 봄",sellerId:MY_ID,si:"나",likes:5,region:"서울 마포구",contact:"안심번호",safeNum:true,tradePlace:"홍대입구역 9번 출구",status:"selling"},
  {id:5,title:"대형 현수막 천",category:["기타"],itemName:"현수막 천",price:0,photos:[],emoji:"🏮",desc:"행사 후 남은 대형 천 소재 현수막.",seller:"문화공간 노들",sellerId:"user5",si:"노",likes:3,region:"서울 동작구",contact:"010-****-7890",safeNum:false,tradePlace:"노들섬 내부",status:"selling"},
  {id:6,title:"무대 음향 장비 세트",category:["장비"],itemName:"음향 장비",price:50000,photos:[],emoji:"🎧",desc:"믹서, 스피커, 마이크 포함.",seller:"극단 파도",sellerId:MY_ID,si:"나",likes:17,region:"서울 중구",contact:"010-****-2345",safeNum:false,tradePlace:"을지로 입구역 근처",status:"selling"},
];
const initJobs = [
  {id:101,title:"조명 디자이너 구합니다",field:"조명",type:"단기",org:"극단 파도",pay:"협의",date:"2025.06.14~06.28",desc:"소극장 연극 조명 설계 및 현장 운영.",location:"대학로",icon:"💡"},
  ...
  {id:103,title:"음향 엔지니어 (상주)",field:"음향",type:"장기",org:"문화공간 노들",pay:"월 230만원~",date:"2025.07.01~",desc:"공연장 상주 음향 엔지니어.",location:"노들섬",icon:"🎧"},
            ...
            {mainTab==="jobs"&&filtJobs.map(job=>(
              <div key={job.id} onClick={()=>{setSelJob(job);go("jobdetail");}} style={{padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer"}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>{jbadge(job.type)}<span style={{fontSize:11,color:"#bbb"}}>{job.field}</span></div>
          ...
          <div style={{flex:1,overflowY:"auto"}}>
            ...
            <div style={{padding:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",marginBottom:5}}>{stBadge(selItem.status)}{selItem.category.map(c=><span key={c} style={{fontSize:10,padding:"1px 6px",borderRadius:8,background:"#f0f0f0",color:"#888"}}>{c}</span>)}</div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:16}}>
            {(postMode==="item"||editItem)?(
              <>
                ...
                <button onClick={submitItem} style={{width:"100%",height:46,borderRadius:12,border:"none",background:form.title?ACCENT:"#ddd",color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer",marginBottom:16}}>{editItem?"수정 완료":"올리기"}</button>
              </>
            ):(
              <>
