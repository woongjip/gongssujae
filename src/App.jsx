...
const REGIONS=["서울 종로구","서울 중구","서울 용산구","서울 성동구","서울 마포구","서울 강남구","서울 서초구","서울 송파구","서울 강동구","서울 관악구","서울 동작구","서울 영등포구","서울 강서구","서울 은평구","서울 서대문구","서울 성북구","서울 노원구","서울 도봉구","서울 강북구","서울 양천구","서울 구로구","서울 금천구","서울 중랑구","서울 광진구","서울 동대문구","부산 중구","부산 서구","부산 동구","부산 영도구","부산 부산진구","부산 동래구","부산 남구","부산 북구","부산 해운대구","대구 중구","대구 동구","대구 서구","인천 중구","인천 동구","인천 미추홀구","인천 연수구","광주 동구","광주 서구","광주 남구","광주 북구","대전 동구","대전 중구","대전 서구","대전 유성구","울산 중구","울산 남구","경기 수원시","경기 성남시","경기 고양시","경기 용인시","경기 부천시","경기 안양시","경기 남양주시"];
const MY_ID="me";
const BASE_TEMPS={user1:38.2,user2:36.5,user3:41.0,user4:35.8,user5:37.5};
...
const initItems=[
  {id:1,title:"빨간 드레스 (2벌)",category:["소품","의상"],itemName:"빨간 드레스",price:0,photos:[],emoji:"🎭",desc:"공연 종료 후 남은 빨간 드레스 2벌. 사이즈 S/M. 세탁 완료.",seller:"극단 파도",sellerId:"user1",si:"파",likes:12,region:"서울 종로구",contact:"010-****-1234",safeNum:false,tradePlace:"대학로 마로니에공원 앞",status:"selling",postType:"nanumi",showTag:"더 헬리콥터쇼 2024",showEndDate:""},
  {id:2,title:"소형 스팟 조명 x3",category:["장비"],itemName:"스팟 조명",price:15000,photos:[],emoji:"💡",desc:"전시 철수 후 남은 LED 스팟 조명 3개. 상태 양호.",seller:"아트스페이스 을지",sellerId:"user2",si:"을",likes:8,region:"서울 중구",contact:"안심번호",safeNum:true,tradePlace:"을지로 3가역 2번 출구",status:"reserved",postType:"nanumi",showTag:"달빛 소나타 전시",showEndDate:""},
  {id:3,title:"나무 의자 세트 (6개)",category:["세트"],itemName:"나무 의자",price:0,photos:[],emoji:"🪑",desc:"무대 소품용 나무 의자 6개. 픽업 가능.",seller:"극단 숲",sellerId:"user3",si:"숲",likes:21,region:"서울 종로구",contact:"010-****-9012",safeNum:false,tradePlace:"대학로 소극장 앞",status:"done",postType:"nanumi",showTag:"시간의 방 연극제",showEndDate:""},
  {id:4,title:"한복 치마 (연두색)",category:["의상"],itemName:"한복 치마",price:8000,photos:[],emoji:"👗",desc:"한복 작업 후 남은 치마 1점. 허리 60cm.",seller:"나",sellerId:MY_ID,si:"나",likes:5,region:"서울 마포구",contact:"안심번호",safeNum:true,tradePlace:"홍대입구역 9번 출구",status:"selling",postType:"nanumi",showTag:"춘향 리믹스",showEndDate:"2026-08-15"},
  {id:5,title:"조명 장비 급구합니다",category:["장비"],itemName:"LED 조명",price:0,photos:[],emoji:"🔦",desc:"다음주 공연에 쓸 소형 LED 조명 2개 구합니다.",seller:"문화공간 노들",sellerId:"user5",si:"노",likes:3,region:"서울 동작구",contact:"010-****-7890",safeNum:false,tradePlace:"노들섬 내부",status:"selling",postType:"guhami",showTag:"",showEndDate:""},
...
export default function App(){
  ...
  const filtItems=useMemo(()=>{
    ...
    if(localFirst&&prof.preferredRegion){const city=prof.preferredRegion.split(" ")[0];l=[...l.filter(i=>i.region?.startsWith(city)),...l.filter(i=>!i.region?.startsWith(city))];}
    return l;
  },[items,cat,q,showTagFilter,localFirst,prof.preferredRegion]);
  ...
  return(
    <div style={{maxWidth:390,margin:"0 auto",fontFamily:"sans-serif",position:"relative",border:"1px solid #e5e5e5",borderRadius:24,overflow:"hidden",background:"#fff",height:700,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <div ref={listRef} style={{flex:1,overflowY:"auto",paddingBottom:64}}>
            {mainTab==="items"&&filtItems.map(item=>{
              ...
              const isLocal=localFirst&&prof.preferredRegion&&item.region?.startsWith(prof.preferredRegion.split(" ")[0]);
              return(
                <div key={item.id} onClick={()=>goDetail(item)} style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer",opacity:item.status==="done"?0.55:1,background:isLocal?"#fafffe":"#fff"}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:"flex",gap:4,flexWrap:"wrap",alignItems:"center",marginBottom:3}}>
                          ...
                          {item.showTag&&<span style={{fontSize:10,padding:"1px 7px",borderRadius:10,background:"#e8eaf6",color:"#3949ab",fontWeight:500}}>🎭 {item.showTag}</span>}
                        </div>
                      </div>
            ...
            {mainTab==="jobs"&&filtJobs.map(job=>(
              <div key={job.id} onClick={()=>{setSelJob(job);go("jobdetail");}} style={{padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer",opacity:job.jobStatus==="done"?0.55:1}}>
                <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3,flexWrap:"wrap"}}>{jbadge(job.type)}{jobBadges(job).map((b,i)=><span key={i}>{b}</span>)}<span style={{fontSize:11,color:"#bbb"}}>{job.field}</span></div>
      ...
      {screen==="detail"&&selItem&&(()=>{
        const badges=itemBadges(selItem);const t=getTemp(selItem.sellerId);
        return(
          <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
            <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {selItem.sellerId===MY_ID&&<button onClick={()=>boostItem(selItem.id)} style={{background:LIGHT,border:"none",borderRadius:8,padding:"4px 10px",fontSize:12,color:ACCENT,cursor:"pointer",fontWeight:500}}>⬆ 끌어올리기</button>}
            ...
            <div style={{flex:1,overflowY:"auto"}}>
              ...
              <div style={{padding:16}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:5,alignItems:"center"}}>{badges.map((b,i)=><span key={i}>{b}</span>)}{showTagPill(selItem.showTag,true)}</div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:16}}>
            {(postMode==="item"||editItem)?(
              <>
                ...
                <div style={{marginBottom:12}}><div style={{fontSize:12,color:"#666",marginBottom:4,fontWeight:500}}>📅 공연 종료일 (알림용)</div><input value={form.showEndDate} onChange={e=>setForm(p=>({...p,showEndDate:e.target.value}))} type="date" style={inp}/></div>
      ...
      {screen==="chatlist"&&(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <div style={{flex:1,overflowY:"auto",paddingBottom:64}}>
            {chatList.map(({id,label})=>{const msgs=chats[id]||[];const last=msgs[msgs.length-1];return(<div key={id} onClick={()=>openChat(id,label)} style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer",alignItems:"center"}}><div style={{width:46,height:46,borderRadius:12,background:LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{id>=100?"💼":"📦"}</div><div style={{flex:1,minWidth:0}}><div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontSize:13,fontWeight:500}}>{label}</div><div style={{fontSize:11,color:"#bbb"}}>방금 전</div></div><div style={{fontSize:12,color:"#999",marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{last?last.text:"새 채팅"}</div></div></div>);})}
          </div>
        </div>
      )}
      ...
      {screen==="chat"&&(
        <div style={{display:"flex",flexDirection:"column",height:"100%",paddingBottom:64}}>
          <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:8,borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}><button onClick={()=>go("chatlist","chatlist")} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#555"}}><i className="ti ti-arrow-left"/></button><div><div style={{fontSize:13,fontWeight:500}}>{chatLabel}</div><div style={{fontSize:11,color:"#aaa"}}>채팅</div></div></div>
      ...
      {screen==="notify"&&(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <div style={{flex:1,overflowY:"auto",padding:16,paddingBottom:80}}>
            {upcomingEnds.length>0&&<div style={{marginBottom:20}}><div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:10}}>📅 공연 종료 예정</div>{upcomingEnds.map(item=>(<div key={item.id} style={{padding:"12px 14px",background:"#fff8e1",borderRadius:12,marginBottom:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:13,fontWeight:500,color:"#1a1a1a"}}>{item.showTag||item.title}</div><div style={{fontSize:11,color:"#f57c00",marginTop:2}}>종료일 {item.showEndDate} · 물건을 올려보세요!</div></div><button onClick={()=>{setShowTagFilter(item.showTag||"");goHome();}} style={{background:"none",border:`1px solid #f57c00`,borderRadius:8,padding:"4px 8px",fontSize:11,color:"#f57c00",cursor:"pointer"}}>보기</button></div>))}</div>}
            <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:10}}>푸시 알림</div>
            {[{k:"comment",l:"내 게시글에 댓글",d:"댓글이 달리면 알림"},{k:"keyword",l:"키워드 알림",d:"등록 키워드 물건 올라오면 알림"},{k:"newItem",l:"새 물건 등록",d:"관심 카테고리 새 물건 알림"},{k:"job",l:"일자리 공고",d:"새 일자리 공고 알림"}].map(({k,l,d})=>(<div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 0",borderBottom:"0.5px solid #f5f5f5"}}><div><div style={{fontSize:14}}>{l}</div><div style={{fontSize:11,color:"#aaa",marginTop:2}}>{d}</div></div><div onClick={()=>setNotify(p=>({...p,[k]:!p[k]}))} style={{width:46,height:27,borderRadius:14,background:notify[k]?ACCENT:"#ddd",cursor:"pointer",position:"relative",flexShrink:0}}><div style={{position:"absolute",top:3,left:notify[k]?21:3,width:21,height:21,borderRadius:"50%",background:"#fff",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.15)"}}/></div></div>))}
        ...
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          <div style={{padding:"18px 16px 0",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
              <div style={{position:"relative"}}><div style={{width:56,height:56,borderRadius:"50%",background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:22,fontWeight:600}}>나</div>{prof.accountType==="단체"&&<span style={{position:"absolute",bottom:-2,right:-2,fontSize:14}}>🏢</span>}</div>
              <div style={{flex:1}}>
                <div style={{fontSize:16,fontWeight:500}}>{prof.affiliation||"공쓰재 사용자"}</div>
                <div style={{fontSize:12,color:"#aaa",marginTop:1}}>{prof.phone||aInput||"010-****-****"}</div>
                <div style={{marginTop:5}}>{tempBadge(MY_ID)}<span style={{fontSize:10,color:"#aaa",marginLeft:6}}>공연온도</span></div>
              </div>
            </div>
      ...
      {reviewModal&&(
        <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"flex-end",zIndex:200}}>
          <div style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:"24px 20px 32px",width:"100%",boxSizing:"border-box"}}>
            <div style={{fontSize:16,fontWeight:600,marginBottom:4}}>거래 후기 — 공연온도</div>
            <div style={{fontSize:13,color:"#888",marginBottom:6}}>"{reviewModal.title}" 거래가 어떠셨나요?</div>
            <div style={{fontSize:11,color:"#bbb",marginBottom:20}}>후기는 상대방의 🌡️ 공연온도에 반영됩니다</div>
            <div style={{display:"flex",gap:12}}>
              <button onClick={()=>submitReview(true)} style={{flex:1,height:52,borderRadius:14,border:"none",background:LIGHT,color:ACCENT,fontSize:15,fontWeight:600,cursor:"pointer"}}>👍 좋았어요<br/><span style={{fontSize:11,fontWeight:400}}>+0.3°C</span></button>
              <button onClick={()=>submitReview(false)} style={{flex:1,height:52,borderRadius:14,border:"none",background:"#fff0f2",color:"#c62828",fontSize:15,fontWeight:600,cursor:"pointer"}}>👎 별로였어요<br/><span style={{fontSize:11,fontWeight:400}}>-0.3°C</span></button>
            </div>
            <button onClick={()=>setReviewModal(null)} style={{width:"100%",marginTop:12,background:"none",border:"none",color:"#aaa",fontSize:13,cursor:"pointer",padding:"8px 0"}}>건너뛰기</button>
          </div>
        </div>
      )}
