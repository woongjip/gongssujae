...
const ACCENT="#2D6A4F",LIGHT="#f0f7f4",MID="#52b788",ADMIN="#1a237e";
...
const BASE_TEMPS={user1:38.2,user2:36.5,user3:41.0,user4:35.8,user5:37.5,user6:36.5,user7:37.1,user8:39.4,user9:36.5,user10:38.8};

const SAMPLE_USERS=[
  {id:"user1",name:"김민준",org:"극단 파도",accountType:"단체",temp:38.2,posts:2,joined:"2025.05.01",status:"active",region:"서울 종로구"},
  {id:"user2",name:"이서연",org:"아트스페이스 을지",accountType:"단체",temp:36.5,posts:1,joined:"2025.05.03",status:"active",region:"서울 중구"},
  {id:"user3",name:"박준서",org:"극단 숲",accountType:"단체",temp:41.0,posts:1,joined:"2025.05.05",status:"active",region:"서울 종로구"},
  {id:"user4",name:"최지우",org:"무용단 봄",accountType:"단체",temp:35.8,posts:1,joined:"2025.05.07",status:"active",region:"서울 마포구"},
  {id:"user5",name:"정하은",org:"문화공간 노들",accountType:"단체",temp:37.5,posts:1,joined:"2025.05.08",status:"active",region:"서울 동작구"},
  {id:"user6",name:"강도윤",org:"프리랜서",accountType:"개인",temp:36.5,posts:0,joined:"2025.05.10",status:"active",region:"서울 강남구"},
  {id:"user7",name:"윤서아",org:"프리랜서",accountType:"개인",temp:37.1,posts:0,joined:"2025.05.12",status:"active",region:"부산 해운대구"},
  {id:"user8",name:"임지호",org:"극단 새벽",accountType:"단체",temp:39.4,posts:0,joined:"2025.05.14",status:"active",region:"서울 마포구"},
  {id:"user9",name:"한수빈",org:"프리랜서",accountType:"개인",temp:36.5,posts:0,joined:"2025.05.15",status:"suspended",region:"대구 중구"},
  {id:"user10",name:"오다은",org:"극장 별빛",accountType:"단체",temp:38.8,posts:0,joined:"2025.05.16",status:"active",region:"서울 서초구"},
  {id:MY_ID,name:"나(테스터)",org:"공쓰재",accountType:"개인",temp:36.5,posts:2,joined:"2025.04.30",status:"active",region:"서울 마포구"},
];

const REPORTS=[
  {id:1,type:"게시글",target:"조명 장비 급구합니다",reporter:"user3",reason:"허위 정보 의심",date:"2025.05.17",status:"검토중"},
  {id:2,type:"사용자",target:"한수빈",reporter:"user1",reason:"거래 후 연락 두절",date:"2025.05.18",status:"처리완료"},
];

const DAILY=[{d:"05/14",v:2},{d:"05/15",v:4},{d:"05/16",v:1},{d:"05/17",v:3},{d:"05/18",v:5},{d:"05/19",v:2},{d:"오늘",v:4}];
...
const initItems=[
  {id:1,title:"빨간 드레스 (2벌)",category:["소품","의상"],itemName:"빨간 드레스",price:0,photos:[],emoji:"🎭",desc:"공연 종료 후 남은 빨간 드레스 2벌.",seller:"극단 파도",sellerId:"user1",si:"파",likes:12,region:"서울 종로구",contact:"010-****-1234",safeNum:false,tradePlace:"대학로 마로니에공원 앞",status:"selling",postType:"nanumi",showTag:"더 헬리콥터쇼 2024",showEndDate:""},
  {id:2,title:"소형 스팟 조명 x3",category:["장비"],itemName:"스팟 조명",price:15000,photos:[],emoji:"💡",desc:"전시 철수 후 남은 LED 스팟 조명 3개.",seller:"아트스페이스 을지",sellerId:"user2",si:"을",likes:8,region:"서울 중구",contact:"안심번호",safeNum:true,tradePlace:"을지로 3가역 2번 출구",status:"reserved",postType:"nanumi",showTag:"달빛 소나타 전시",showEndDate:""},
  {id:3,title:"나무 의자 세트 (6개)",category:["세트"],itemName:"나무 의자",price:0,photos:[],emoji:"🪑",desc:"무대 소품용 나무 의자 6개.",seller:"극단 숲",sellerId:"user3",si:"숲",likes:21,region:"서울 종로구",contact:"010-****-9012",safeNum:false,tradePlace:"대학로 소극장 앞",status:"done",postType:"nanumi",showTag:"시간의 방 연극제",showEndDate:""},
  {id:4,title:"한복 치마 (연두색)",category:["의상"],itemName:"한복 치마",price:8000,photos:[],emoji:"👗",desc:"한복 작업 후 남은 치마 1점.",seller:"나",sellerId:MY_ID,si:"나",likes:5,region:"서울 마포구",contact:"안심번호",safeNum:true,tradePlace:"홍대입구역 9번 출구",status:"selling",postType:"nanumi",showTag:"춘향 리믹스",showEndDate:"2026-08-15"},
  {id:5,title:"조명 장비 급구합니다",category:["장비"],itemName:"LED 조명",price:0,photos:[],emoji:"🔦",desc:"소형 LED 조명 2개 구합니다.",seller:"문화공간 노들",sellerId:"user5",si:"노",likes:3,region:"서울 동작구",contact:"010-****-7890",safeNum:false,tradePlace:"노들섬 내부",status:"selling",postType:"guhami",showTag:"",showEndDate:""},
  {id:6,title:"무대 음향 장비 세트",category:["장비"],itemName:"음향 장비",price:50000,photos:[],emoji:"🎧",desc:"믹서, 스피커, 마이크 포함.",seller:"나",sellerId:MY_ID,si:"나",likes:17,region:"서울 중구",contact:"010-****-2345",safeNum:false,tradePlace:"을지로 입구역 근처",status:"selling",postType:"nanumi",showTag:"할로윈 퍼포먼스",showEndDate:"2026-09-01"},
];
const initJobs=[
  ...
  {id:103,title:"음향 엔지니어 구직합니다",field:"음향",type:"장기",org:"홍길동",pay:"협의",date:"2025.07~",desc:"공연장 음향 엔지니어 경력 5년.",location:"서울 전체",icon:"🎧",jobType:"gujik",sellerId:"user3",jobStatus:"active"},
...
const initChats={1:[{from:"other",text:"안녕하세요! 드레스 아직 남아있나요?"},{from:"me",text:"네, 아직 있어요!"}],101:[{from:"other",text:"조명 디자이너 공고 관련해서 문의드려요."}]};
...
export default function App(){
  ...
  const [prof,setProf]=useState({phone:"",address:"",affiliation:"",interests:[],accountType:"개인",preferredRegion:"",temp:36.5});
  ...
  const filtItems=useMemo(()=>{let l=cat==="전체"?items:items.filter(i=>i.category.includes(cat));if(showTagFilter)l=l.filter(i=>i.showTag===showTagFilter);if(q)l=l.filter(i=>i.title.includes(q)||i.itemName?.includes(q)||i.region?.includes(q)||i.showTag?.includes(q));if(localFirst&&prof.preferredRegion){const city=prof.preferredRegion.split(" ")[0];l=[...l.filter(i=>i.region?.startsWith(city)),...l.filter(i=>!i.region?.startsWith(city))];}return l;},[items,cat,q,showTagFilter,localFirst,prof.preferredRegion]);
  ...
  const likedItems=useMemo(()=>items.filter(i=>likes[i.id]),[items,likes]);
  const myItems=useMemo(()=>items.filter(i=>i.sellerId===MY_ID),[items]);
  const upcomingEnds=useMemo(()=>items.filter(i=>i.showEndDate&&new Date(i.showEndDate)>=new Date()),[items]);
  const filtAdminUsers=useMemo(()=>adminUserQ?users.filter(u=>u.name.includes(adminUserQ)||u.org.includes(adminUserQ)):users,[users,adminUserQ]);
  ...
  const adminStats=useMemo(()=>({totalUsers:users.length,activeUsers:users.filter(u=>u.status==="active").length,totalItems:items.length,doneItems:items.filter(i=>i.status==="done").length,totalJobs:jobs.length,reports:reports.filter(r=>r.status==="검토중").length}),[users,items,jobs,reports]);

  const getTemp=sid=>sid===MY_ID?prof.temp:(BASE_TEMPS[sid]||36.5);
  ...
  function submitItem(){if(!form.title)return;const ni={...(editItem||{}),id:editItem?editItem.id:Date.now(),...form,price:form.price?parseInt(form.price):0,seller:prof.affiliation||"나",sellerId:MY_ID,si:"나",likes:editItem?.likes||0,emoji:form.photos.length===0?"📦":null};if(editItem){setItems(p=>p.map(i=>i.id===editItem.id?ni:i));setSelItem(ni);}else setItems(p=>[ni,...p]);setEditItem(null);setForm(emptyForm);setPosted(true);setTimeout(()=>{setPosted(false);go(editItem?"detail":"home",editItem?undefined:"home");},1200);}
  function startEdit(item){setForm({title:item.title,category:item.category,itemName:item.itemName||"",price:item.price.toString(),desc:item.desc,region:item.region,contact:item.contact,safeNum:item.safeNum,tradePlace:item.tradePlace,photos:item.photos||[],status:item.status,postType:item.postType||"nanumi",showTag:item.showTag||"",showEndDate:item.showEndDate||""});setEditItem(item);setPostMode("item");go("post","post");}
  function changeStatus(id,s){setItems(p=>p.map(i=>i.id===id?{...i,status:s}:i));setSelItem(p=>p?{...p,status:s}:p);if(s==="done"){const item=items.find(i=>i.id===id);if(item)setTimeout(()=>setReviewModal(item),400);}}
  function boostItem(id){setItems(p=>{const item=p.find(i=>i.id===id);if(!item)return p;return[{...item},...p.filter(i=>i.id!==id)];});setBoostToast(true);setTimeout(()=>setBoostToast(false),2000);}
  function submitReview(positive){const delta=positive?0.3:-0.3;setProf(p=>({...p,temp:Math.max(30,Math.min(50,+(p.temp+delta).toFixed(1)))}));setReviewModal(null);}
  function submitJob(){if(!jform.title)return;const nj={id:editJob?editJob.id:Date.now(),...jform,org:editJob?editJob.org:(prof.affiliation||"나"),icon:editJob?editJob.icon:"📋",sellerId:MY_ID};if(editJob)setJobs(p=>p.map(j=>j.id===editJob.id?nj:j));else setJobs(p=>[nj,...p]);setEditJob(null);setJform(emptyJform);setPosted(true);setTimeout(()=>{setPosted(false);setMainTab("jobs");goHome();},1200);}
  ...
  const tempBadge=(sid)=>{const t=getTemp(sid);return <span style={{fontSize:11,color:tempColor(t),fontWeight:600}}>🌡️ {t.toFixed(1)}°C</span>;};
  ...
  const StatCard=({label,value,color="#1a1a1a",sub})=>(
    <div style={{flex:"1 1 calc(33% - 8px)",background:"#f9f9f9",borderRadius:12,padding:"12px 10px",minWidth:80}}>
  ...
  if(!loggedIn){
    ...
    if(aStep==="splash")return(<div style={{maxWidth:390,margin:"0 auto",fontFamily:"sans-serif",border:"1px solid #e5e5e5",borderRadius:24,overflow:"hidden",background:"#fff",height:700,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,boxSizing:"border-box"}}><div style={{fontSize:64,marginBottom:16}}>🎭</div><div style={{fontSize:28,fontWeight:600,color:ACCENT,marginBottom:8}}>공쓰재</div><div style={{fontSize:13,color:"#999",textAlign:"center",lineHeight:1.7,marginBottom:48}}>공연에 쓰고 남은 물건과 일자리를<br/>나누는 공연인들의 플랫폼</div><button onClick={()=>setAStep("login")} style={{width:"100%",height:50,borderRadius:14,border:"none",background:ACCENT,color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer",marginBottom:12}}>시작하기</button><button onClick={()=>setAStep("login")} style={{width:"100%",height:50,borderRadius:14,border:`1px solid ${ACCENT}`,background:"#fff",color:ACCENT,fontSize:15,fontWeight:500,cursor:"pointer"}}>이미 계정이 있어요</button></div>);
    if(aStep==="login")return wrap(<>{backBtn("splash")}<div style={{fontSize:22,fontWeight:600,marginBottom:6}}>안녕하세요!</div><div style={{fontSize:13,color:"#999",marginBottom:24}}>전화번호 또는 이메일로 시작하세요</div><div style={{display:"flex",gap:8,marginBottom:16}}>{[["phone","📱 전화번호"],["email","✉️ 이메일"]].map(([t,l])=>(<button key={t} onClick={()=>{setAMode(t);setAInput("");}} style={{flex:1,padding:"10px 0",borderRadius:10,border:`1px solid ${aMode===t?ACCENT:"#e0e0e0"}`,background:aMode===t?LIGHT:"#fff",color:aMode===t?ACCENT:"#888",fontSize:13,cursor:"pointer",fontWeight:aMode===t?500:400}}>{l}</button>))}</div><input value={aInput} onChange={e=>setAInput(e.target.value)} placeholder={aMode==="phone"?"010-0000-0000":"이메일 주소"} type={aMode==="phone"?"tel":"email"} style={{...inp,marginBottom:12}}/><button onClick={()=>{if(aInput.length>=6)setAStep("verify");}} style={{width:"100%",height:48,borderRadius:12,border:"none",background:aInput.length>=6?ACCENT:"#ddd",color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer"}}>인증번호 받기</button></>);
    if(aStep==="verify")return wrap(<>{backBtn("login")}<div style={{fontSize:22,fontWeight:600,marginBottom:6}}>인증번호 입력</div><div style={{fontSize:13,color:"#999",marginBottom:28}}>{aInput}로 발송된 인증번호 4자리</div><div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:20}}>{[0,1,2,3].map(i=>(<div key={i} style={{width:60,height:60,borderRadius:14,border:`2px solid ${verCode.length>i?ACCENT:"#e0e0e0"}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:600,color:ACCENT,background:verCode.length>i?LIGHT:"#fff"}}>{verCode[i]||""}</div>))}</div><input value={verCode} onChange={e=>setVerCode(e.target.value.replace(/\D/g,"").slice(0,4))} type="tel" style={{...inp,textAlign:"center",fontSize:20,letterSpacing:12,marginBottom:12}}/><button onClick={()=>setAStep("terms")} style={{width:"100%",height:48,borderRadius:12,border:"none",background:ACCENT,color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer",marginBottom:10}}>확인</button></>);
    if(aStep==="terms")return wrap(<><div style={{fontSize:20,fontWeight:600,marginBottom:4}}>약관 동의</div><div style={{fontSize:13,color:"#999",marginBottom:18}}>서비스 이용을 위해 약관에 동의해주세요</div><div onClick={()=>allTerms(!terms.all)} style={{display:"flex",alignItems:"center",gap:10,padding:"14px 16px",background:LIGHT,borderRadius:12,marginBottom:12,cursor:"pointer"}}><div style={{width:22,height:22,borderRadius:22,background:terms.all?ACCENT:"#fff",border:`1.5px solid ${terms.all?ACCENT:"#ccc"}`,display:"flex",alignItems:"center",justifyContent:"center"}}>{terms.all&&<i className="ti ti-check" style={{fontSize:13,color:"#fff"}}/>}</div><span style={{fontSize:14,fontWeight:500,color:ACCENT}}>전체 동의</span></div><div style={{flex:1,overflowY:"auto"}}>{[{k:"service",l:"[필수] 중고물품 거래 이용약관",d:"거래 당사자 간 책임, 분쟁 해결, 금지 품목을 규정합니다."},{k:"privacy",l:"[필수] 개인정보 수집 및 이용 동의",d:"연락처 등을 서비스 제공 목적으로 수집·이용합니다."},{k:"age",l:"[필수] 만 14세 이상 확인",d:"본 서비스는 만 14세 이상만 이용 가능합니다."}].map(({k,l,d})=>(<div key={k} style={{marginBottom:10}}><div onClick={()=>setTerms(p=>({...p,[k]:!p[k],all:false}))} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",padding:"8px 0"}}><div style={{width:20,height:20,borderRadius:20,background:terms[k]?ACCENT:"#fff",border:`1.5px solid ${terms[k]?ACCENT:"#ccc"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{terms[k]&&<i className="ti ti-check" style={{fontSize:12,color:"#fff"}}/>}</div><span style={{fontSize:13,color:"#333"}}>{l}</span></div><div style={{fontSize:11,color:"#aaa",padding:"0 0 4px 30px",lineHeight:1.6}}>{d}</div></div>))}</div><button onClick={()=>{if(terms.service&&terms.privacy&&terms.age)setAStep("profile");}} style={{width:"100%",height:48,borderRadius:12,border:"none",background:(terms.service&&terms.privacy&&terms.age)?ACCENT:"#ddd",color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer",marginTop:12}}>다음</button></>);
    return wrap(<><div style={{fontSize:20,fontWeight:600,marginBottom:4}}>프로필 등록</div><div style={{fontSize:13,color:"#999",marginBottom:18}}>더 나은 서비스를 위해 입력해주세요 (선택)</div><div style={{flex:1,overflowY:"auto"}}>{[{l:"전화번호",k:"phone",ph:"010-0000-0000",t:"tel"},{l:"주소",k:"address",ph:"예: 서울 마포구"},{l:"소속",k:"affiliation",ph:"예: 극단 파도"}].map(f=>(<div key={f.k} style={{marginBottom:14}}><div style={{fontSize:12,color:"#666",marginBottom:5,fontWeight:500}}>{f.l}</div><input value={prof[f.k]} onChange={e=>setProf(p=>({...p,[f.k]:e.target.value}))} placeholder={f.ph} type={f.t||"text"} style={inp}/></div>))}<div style={{marginBottom:14}}><div style={{fontSize:12,color:"#666",marginBottom:6,fontWeight:500}}>계정 유형</div><div style={{display:"flex",gap:8}}>{[["개인","👤"],["단체","🏢"]].map(([t,icon])=>(<button key={t} onClick={()=>setProf(p=>({...p,accountType:t}))} style={{flex:1,padding:"10px 0",borderRadius:12,border:`1.5px solid ${prof.accountType===t?ACCENT:"#e0e0e0"}`,background:prof.accountType===t?LIGHT:"#fff",color:prof.accountType===t?ACCENT:"#888",fontSize:13,cursor:"pointer",fontWeight:prof.accountType===t?600:400}}>{icon} {t}</button>))}</div></div><div style={{marginBottom:14}}><div style={{fontSize:12,color:"#666",marginBottom:8,fontWeight:500}}>관심 분야</div><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{INTERESTS.map(i=>{const a=prof.interests.includes(i);return(<button key={i} onClick={()=>setProf(p=>({...p,interests:a?p.interests.filter(x=>x!==i):[...p.interests,i]}))} style={{padding:"6px 14px",borderRadius:20,border:"0.5px solid",borderColor:a?ACCENT:"#e0e0e0",background:a?ACCENT:"#fff",color:a?"#fff":"#555",fontSize:13,cursor:"pointer"}}>{i}</button>);})}</div></div></div><div style={{display:"flex",gap:8,marginTop:12}}><button onClick={()=>setLoggedIn(true)} style={{flex:1,height:48,borderRadius:12,border:"0.5px solid #e0e0e0",background:"#fff",color:"#888",fontSize:14,cursor:"pointer"}}>건너뛰기</button><button onClick={()=>setLoggedIn(true)} style={{flex:2,height:48,borderRadius:12,border:"none",background:ACCENT,color:"#fff",fontSize:15,fontWeight:500,cursor:"pointer"}}>완료</button></div></>);
  }
  ...
  return(
    <div style={{maxWidth:390,margin:"0 auto",fontFamily:"sans-serif",position:"relative",border:"1px solid #e5e5e5",borderRadius:24,overflow:"hidden",background:"#fff",height:700,display:"flex",flexDirection:"column"}}>
      ...
      {screen==="home"&&(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          ...
          <div style={{padding:"8px 16px",borderBottom:"0.5px solid #f5f5f5",overflowX:"auto",display:"flex",gap:6,flexShrink:0}}>
            {mainTab==="items"?<>{ITEM_CATS.map(c=>chip(c,cat===c,()=>setCat(c)))}{prof.preferredRegion&&chip(localFirst?"📍 내 지역 ON":"📍 내 지역 먼저",localFirst,()=>setLocalFirst(l=>!l))}</>:JOB_FIELDS.map(f=>chip(f,fld===f,()=>setFld(f)))}
          </div>
          <div ref={listRef} style={{flex:1,overflowY:"auto",paddingBottom:64}}>
            {mainTab==="items"&&filtItems.map(item=>{
              const badges=itemBadges(item);const isLocal=localFirst&&prof.preferredRegion&&item.region?.startsWith(prof.preferredRegion.split(" ")[0]);
              return(<div key={item.id} onClick={()=>goDetail(item)} style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer",opacity:item.status==="done"?0.55:1,background:isLocal?"#fafffe":"#fff"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:13,fontWeight:500,color:item.price===0&&item.postType!=="guhami"?ACCENT:"#1a1a1a"}}>{item.postType==="guhami"?(item.price===0?"가격 협의":`예산 ${item.price.toLocaleString()}원`):(item.price===0?"무료 나눔":`${item.price.toLocaleString()}원`)}</span><span style={{fontSize:10,color:"#ccc"}}><i className="ti ti-heart" style={{fontSize:11,verticalAlign:-1,marginRight:2}}/>{item.likes+(likes[item.id]?1:0)}</span></div>
                </div>
              </div>);
            })}
      ...
      {/* 물건 상세 */}
      {screen==="detail"&&selItem&&(()=>{const badges=itemBadges(selItem);return(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}>
            ...
            <div style={{display:"flex",gap:8,alignItems:"center"}}>{selItem.sellerId===MY_ID&&<button onClick={()=>boostItem(selItem.id)} style={{background:LIGHT,border:"none",borderRadius:8,padding:"4px 10px",fontSize:12,color:ACCENT,cursor:"pointer",fontWeight:500}}>⬆ 끌어올리기</button>}{selItem.sellerId===MY_ID&&<button onClick={()=>startEdit(selItem)} style={{background:"none",border:"none",fontSize:13,cursor:"pointer",color:ACCENT,fontWeight:600}}>수정</button>}</div>
          </div>
          <div style={{flex:1,overflowY:"auto"}}>
            ...
            <div style={{padding:16}}>
              ...
              <div style={{border:"0.5px solid #f0f0f0",borderRadius:12,overflow:"hidden",marginBottom:12}}><div style={{padding:"12px 14px 8px"}}><div style={{fontSize:11,color:"#aaa",marginBottom:4}}>거래 희망 장소</div><div style={{fontSize:14,display:"flex",alignItems:"center",gap:6}}><i className="ti ti-map-pin" style={{fontSize:15,color:ACCENT}}/>{selItem.tradePlace}</div></div><div style={{height:90,background:LIGHT,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6}}><div style={{fontSize:22}}>🗺️</div><a href={`https://map.naver.com/v5/search/${encodeURIComponent(selItem.tradePlace||"")}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:ACCENT,textDecoration:"none",border:`1px solid ${ACCENT}`,padding:"4px 14px",borderRadius:12}}>지도에서 보기</a></div></div>
              <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",border:"0.5px solid #f0f0f0",borderRadius:12}}><div style={{width:40,height:40,borderRadius:"50%",background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:500,fontSize:14}}>{selItem.si}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{selItem.seller}</div><div style={{fontSize:11,color:"#aaa"}}>판매자</div></div>{tempBadge(selItem.sellerId)}</div>
            </div>
          </div>
          <div style={{padding:"12px 16px 80px",borderTop:"0.5px solid #f0f0f0",flexShrink:0}}><button onClick={()=>openChat(selItem.id,selItem.title)} style={{width:"100%",height:54,borderRadius:14,border:"none",background:ACCENT,color:"#fff",fontSize:17,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}><i className="ti ti-message-circle" style={{fontSize:24}}/>판매자와 채팅하기</button></div>
        </div>
      );})()}

      {/* 공고 상세 */}
      {screen==="jobdetail"&&selJob&&(()=>{const badges=jobBadges(selJob);return(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          <div style={{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}><div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={goHome} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#555"}}><i className="ti ti-arrow-left"/></button><span style={{fontWeight:500,fontSize:15}}>공고 상세</span></div>{selJob.sellerId===MY_ID&&<button onClick={()=>startEditJob(selJob)} style={{background:"none",border:"none",fontSize:13,cursor:"pointer",color:ACCENT,fontWeight:600}}>수정</button>}</div>
          <div style={{flex:1,overflowY:"auto",padding:16}}>
            ...
            <div style={{marginTop:16,padding:14,background:"#fafafa",borderRadius:12,marginBottom:12}}><p style={{margin:0,fontSize:14,lineHeight:1.7,color:"#333"}}>{selJob.desc}</p></div>
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",border:"0.5px solid #f0f0f0",borderRadius:12}}><div style={{width:40,height:40,borderRadius:"50%",background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:14,fontWeight:500}}>{selJob.org[0]}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{selJob.org}</div><div style={{fontSize:11,color:"#aaa"}}>등록자</div></div>{tempBadge(selJob.sellerId)}</div>
          </div>
          <div style={{padding:"12px 16px 80px",borderTop:"0.5px solid #f0f0f0",flexShrink:0}}><button onClick={()=>openChat(selJob.id,selJob.title)} style={{width:"100%",height:54,borderRadius:14,border:"none",background:ACCENT,color:"#fff",fontSize:17,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}><i className="ti ti-message-circle" style={{fontSize:24}}/>지원 / 문의 채팅</button></div>
        </div>
      );})()}
      ...
      {/* 채팅 목록 */}
      {screen==="chatlist"&&(<div style={{display:"flex",flexDirection:"column",height:"100%"}}><div style={{padding:"20px 16px 14px",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}><div style={{fontSize:18,fontWeight:500}}>채팅</div></div><div style={{flex:1,overflowY:"auto",paddingBottom:64}}>{chatList.map(({id,label})=>{const msgs=chats[id]||[];const last=msgs[msgs.length-1];return(<div key={id} onClick={()=>openChat(id,label)} style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer",alignItems:"center"}}><div style={{width:46,height:46,borderRadius:12,background:LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{id>=100?"💼":"📦"}</div><div style={{flex:1,minWidth:0}}><div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontSize:13,fontWeight:500}}>{label}</div><div style={{fontSize:11,color:"#bbb"}}>방금 전</div></div><div style={{fontSize:12,color:"#999",marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{last?last.text:"새 채팅"}</div></div></div>);})}</div></div>)}

      {/* 채팅 */}
      {screen==="chat"&&(<div style={{display:"flex",flexDirection:"column",height:"100%",paddingBottom:64}}><div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:8,borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}><button onClick={()=>go("chatlist","chatlist")} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:"#555"}}><i className="ti ti-arrow-left"/></button><div><div style={{fontSize:13,fontWeight:500}}>{chatLabel}</div><div style={{fontSize:11,color:"#aaa"}}>채팅</div></div></div><div style={{flex:1,overflowY:"auto",padding:"12px 16px",display:"flex",flexDirection:"column",gap:10}}>{(chats[activeChat]||[]).map((msg,i)=>(<div key={i} style={{display:"flex",justifyContent:msg.from==="me"?"flex-end":"flex-start"}}><div style={{maxWidth:"72%",padding:"10px 14px",borderRadius:msg.from==="me"?"18px 18px 4px 18px":"18px 18px 18px 4px",background:msg.from==="me"?ACCENT:"#f3f3f3",color:msg.from==="me"?"#fff":"#1a1a1a",fontSize:14,lineHeight:1.5}}>{msg.text}</div></div>))}{!(chats[activeChat]||[]).length&&<div style={{textAlign:"center",color:"#ccc",fontSize:13,marginTop:40}}>메시지를 보내보세요</div>}<div ref={chatEnd}/></div><div style={{flexShrink:0,padding:"10px 12px",borderTop:"1px solid #f0f0f0",display:"flex",gap:8,alignItems:"center",background:"#fff"}}><input value={chatMsg} onChange={e=>setChatMsg(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg()} placeholder="메시지를 입력하세요" style={{flex:1,borderRadius:22,border:"1px solid #e0e0e0",padding:"11px 16px",fontSize:14,outline:"none",background:"#fafafa"}}/><button onClick={sendMsg} style={{width:44,height:44,borderRadius:"50%",border:"none",background:chatMsg.trim()?ACCENT:"#ddd",color:"#fff",fontSize:20,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><i className="ti ti-send"/></button></div></div>)}
      ...
      {/* 마이페이지 */}
      {screen==="mypage"&&(<div style={{display:"flex",flexDirection:"column",height:"100%"}}><div style={{padding:"18px 16px 0",borderBottom:"0.5px solid #f0f0f0",flexShrink:0}}><div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}><div style={{position:"relative"}}><div style={{width:56,height:56,borderRadius:"50%",background:ACCENT,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:22,fontWeight:600}}>나</div>{prof.accountType==="단체"&&<span style={{position:"absolute",bottom:-2,right:-2,fontSize:14}}>🏢</span>}</div><div style={{flex:1}}><div style={{fontSize:16,fontWeight:500}}>{prof.affiliation||"공쓰재 사용자"}</div><div style={{fontSize:12,color:"#aaa",marginTop:1}}>{prof.phone||aInput||"010-****-****"}</div><div style={{marginTop:5}}>{tempBadge(MY_ID)}<span style={{fontSize:10,color:"#aaa",marginLeft:6}}>공연온도</span></div></div></div><div style={{display:"flex"}}>{[["info","정보"],["liked","찜 목록"],["myitems","내 게시글"]].map(([t,l])=>(<button key={t} onClick={()=>setMypageTab(t)} style={{flex:1,padding:"8px 0",border:"none",background:"none",cursor:"pointer",fontSize:12,fontWeight:mypageTab===t?500:400,color:mypageTab===t?ACCENT:"#aaa",borderBottom:mypageTab===t?`2px solid ${ACCENT}`:"2px solid transparent"}}>{l}</button>))}</div></div>
        <div style={{flex:1,overflowY:"auto",paddingBottom:64}}>
          {mypageTab==="info"&&(<><div style={{padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5"}}><div style={{fontSize:12,color:"#aaa",marginBottom:8}}>계정 유형</div><div style={{display:"flex",gap:8}}>{[["개인","👤"],["단체","🏢"]].map(([t,icon])=>(<button key={t} onClick={()=>setProf(p=>({...p,accountType:t}))} style={{flex:1,padding:"8px 0",borderRadius:10,border:`1.5px solid ${prof.accountType===t?ACCENT:"#e0e0e0"}`,background:prof.accountType===t?LIGHT:"#fff",color:prof.accountType===t?ACCENT:"#888",fontSize:13,cursor:"pointer",fontWeight:prof.accountType===t?600:400}}>{icon} {t}</button>))}</div></div><div style={{padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5"}}><div style={{fontSize:12,color:"#aaa",marginBottom:6}}>📍 선호 지역</div><div style={{position:"relative"}}><input value={prof.preferredRegion||"지역 선택"} readOnly onClick={()=>setShowPrefR(true)} style={{...inp,cursor:"pointer",fontSize:13,color:prof.preferredRegion?"#1a1a1a":"#aaa"}}/>{showPrefR&&(<div style={{position:"absolute",top:"100%",left:0,right:0,background:"#fff",border:"1px solid #e0e0e0",borderRadius:10,zIndex:100,maxHeight:160,overflowY:"auto",boxShadow:"0 4px 16px rgba(0,0,0,0.1)"}}><div style={{padding:"8px 12px",borderBottom:"0.5px solid #f0f0f0",position:"sticky",top:0,background:"#fff"}}><input value={prefRSearch} onChange={e=>setPrefRSearch(e.target.value)} placeholder="지역 검색" style={{width:"100%",border:"none",outline:"none",fontSize:13}} autoFocus/></div><div onClick={()=>{setProf(p=>({...p,preferredRegion:""}));setShowPrefR(false);}} style={{padding:"10px 12px",fontSize:13,cursor:"pointer",color:"#aaa",borderBottom:"0.5px solid #f9f9f9"}}>선택 안함</div>{filtPrefR.slice(0,20).map(r=>(<div key={r} onClick={()=>{setProf(p=>({...p,preferredRegion:r}));setShowPrefR(false);setPrefRSearch("");}} style={{padding:"10px 12px",fontSize:13,cursor:"pointer",borderBottom:"0.5px solid #f9f9f9",background:prof.preferredRegion===r?LIGHT:"#fff",color:prof.preferredRegion===r?ACCENT:"#333"}}>{r}</div>))}</div>)}</div></div>{[["알림 설정","ti-bell"],["거래 내역","ti-repeat"]].map(([l,ic])=>(<div key={l} onClick={()=>{if(l==="알림 설정")go("notify","");}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",gap:10}}><i className={`ti ${ic}`} style={{fontSize:18,color:"#555"}}/><span style={{fontSize:14}}>{l}</span></div><i className="ti ti-chevron-right" style={{fontSize:16,color:"#ccc"}}/></div>))}<div onClick={()=>{setLoggedIn(false);setAStep("splash");}} style={{display:"flex",alignItems:"center",gap:10,padding:"16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer"}}><i className="ti ti-logout" style={{fontSize:18,color:"#e25"}}/><span style={{fontSize:14,color:"#e25"}}>로그아웃</span></div><div onClick={()=>setShowAdminLogin(true)} style={{padding:"12px 16px",textAlign:"center",cursor:"pointer"}}><span style={{fontSize:11,color:"#ddd"}}>관리자</span></div></>)}
          {mypageTab==="liked"&&(likedItems.length===0?<div style={{textAlign:"center",color:"#ccc",marginTop:60,fontSize:14}}>찜한 물건이 없어요</div>:likedItems.map(item=>(<div key={item.id} onClick={()=>goDetail(item)} style={{display:"flex",gap:12,padding:"14px 16px",borderBottom:"0.5px solid #f5f5f5",cursor:"pointer"}}><div style={{width:64,height:64,borderRadius:10,flexShrink:0,overflow:"hidden",background:LIGHT,display:"flex",alignItems:"center",justifyContent:"center"}}>{item.photos?.length>0?<img src={item.photos[0]} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<span style={{fontSize:24}}>{item.emoji||"📦"}</span>}</div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:500,marginBottom:3}}>{item.title}</div><div style={{fontSize:11,color:"#bbb"}}><i className="ti ti-map-pin" style={{fontSize:10,marginRight:2}}/>{item.region}</div><div style={{fontSize:13,fontWeight:500,color:item.price===0?ACCENT:"#1a1a1a",marginTop:4}}>{item.price===0?"무료 나눔":`${item.price.toLocaleString()}원`}</div></div><button onClick={e=>{e.stopPropagation();setLikes(p=>({...p,[item.id]:false}));}} style={{background:"none",border:"none",cursor:"pointer",color:"#e25",fontSize:18,alignSelf:"center",flexShrink:0}}><i className="ti ti-heart-filled"/></button></div>)))}
      ...
      {screen==="admin"&&(
        <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
          {/* 관리자 헤더 */}
          <div style={{padding:"14px 16px",background:ADMIN,display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
            <div><div style={{fontSize:15,fontWeight:600,color:"#fff"}}>🔐 관리자 모드</div><div style={{fontSize:11,color:"rgba(255,255,255,0.6)",marginTop:1}}>공쓰재 Admin Dashboard</div></div>
            <button onClick={()=>{setIsAdmin(false);setScreen("home");setBtab("home");}} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,padding:"6px 12px",color:"#fff",fontSize:12,cursor:"pointer"}}>나가기</button>
          </div>
          {/* 관리자 탭 */}
          <div style={{display:"flex",background:"#f8f9ff",borderBottom:"0.5px solid #e0e0e0",flexShrink:0}}>
            {[["dashboard","대시보드"],["users","회원"],["posts","게시글"],["stats","통계"],["reports","신고"]].map(([t,l])=>(<button key={t} onClick={()=>setAdminTab(t)} style={{flex:1,padding:"10px 0",border:"none",background:"none",cursor:"pointer",fontSize:11,fontWeight:adminTab===t?600:400,color:adminTab===t?ADMIN:"#888",borderBottom:adminTab===t?`2px solid ${ADMIN}`:"2px solid transparent"}}>{l}{t==="reports"&&reports.filter(r=>r.status==="검토중").length>0&&<span style={{marginLeft:3,fontSize:9,background:"#e25",color:"#fff",borderRadius:8,padding:"1px 4px"}}>{reports.filter(r=>r.status==="검토중").length}</span>}</button>))}
          </div>
          <div style={{flex:1,overflowY:"auto",padding:16,paddingBottom:20}}>
            ...
            {adminTab==="dashboard"&&(<>
              <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:20}}>
                <StatCard label="총 회원" value={adminStats.totalUsers} color={ADMIN}/>
                <StatCard label="활성 회원" value={adminStats.activeUsers} color="#2e7d32"/>
                <StatCard label="총 게시글" value={adminStats.totalItems}/>
                <StatCard label="거래완료" value={adminStats.doneItems} color="#e65100"/>
                <StatCard label="일자리 공고" value={adminStats.totalJobs}/>
                <StatCard label="신고 접수" value={adminStats.reports} color={adminStats.reports>0?"#c62828":"#9e9e9e"}/>
              </div>
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:10}}>📈 일별 게시글 등록</div>
                {(()=>{const max=Math.max(...DAILY.map(d=>d.v),1);return DAILY.map(d=>(<div key={d.d} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><div style={{width:36,fontSize:10,color:"#888",textAlign:"right",flexShrink:0}}>{d.d}</div><div style={{flex:1,height:18,background:"#f0f0f0",borderRadius:9,overflow:"hidden"}}><div style={{height:"100%",width:`${(d.v/max)*100}%`,background:ADMIN,borderRadius:9}}/></div><div style={{width:14,fontSize:11,color:"#555",fontWeight:500,textAlign:"right"}}>{d.v}</div></div>));})()}
              </div>
              <div style={{marginBottom:16}}>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:10}}>👥 최근 가입 회원</div>
                {users.slice(-3).reverse().map(u=>(<div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"0.5px solid #f5f5f5"}}><div style={{width:32,height:32,borderRadius:"50%",background:u.status==="suspended"?"#f5f5f5":LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>{u.accountType==="단체"?"🏢":"👤"}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:500}}>{u.name}<span style={{fontSize:10,color:"#aaa",marginLeft:6}}>{u.org}</span></div><div style={{fontSize:11,color:"#bbb"}}>{u.joined}</div></div><span style={{fontSize:10,padding:"2px 8px",borderRadius:10,background:u.status==="active"?"#e8f5e9":"#ffebee",color:u.status==="active"?"#2e7d32":"#c62828",fontWeight:500}}>{u.status==="active"?"활성":"정지"}</span></div>))}
              </div>
            </>)}

            {/* 회원 관리 */}
            {adminTab==="users"&&(<>
              <input value={adminUserQ} onChange={e=>setAdminUserQ(e.target.value)} placeholder="이름 또는 소속 검색" style={{...inp,marginBottom:14}}/>
              <div style={{fontSize:12,color:"#aaa",marginBottom:8}}>총 {filtAdminUsers.length}명</div>
              {filtAdminUsers.map(u=>(<div key={u.id} style={{padding:"12px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:u.status==="suspended"?"#f5f5f5":LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>{u.accountType==="단체"?"🏢":"👤"}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:500}}>{u.name}<span style={{fontSize:10,color:"#aaa",marginLeft:6}}>{u.accountType}</span></div>
                    <div style={{fontSize:11,color:"#888"}}>{u.org} · {u.region}</div>
                    <div style={{display:"flex",gap:8,marginTop:3,alignItems:"center"}}>
                      <span style={{fontSize:10,color:tempColor(u.temp),fontWeight:600}}>🌡️ {u.temp}°C</span>
                      <span style={{fontSize:10,color:"#aaa"}}>게시글 {u.posts}개</span>
                      <span style={{fontSize:10,color:"#aaa"}}>가입 {u.joined}</span>
                    </div>
                  </div>
                  {u.id!==MY_ID&&<button onClick={()=>toggleUserStatus(u.id)} style={{background:u.status==="active"?"#ffebee":"#e8f5e9",border:"none",borderRadius:8,padding:"5px 10px",fontSize:11,color:u.status==="active"?"#c62828":"#2e7d32",cursor:"pointer",fontWeight:500,flexShrink:0}}>{u.status==="active"?"정지":"활성화"}</button>}
                </div>
              </div>))}
            </>)}
            ...
            {/* 게시글 관리 */}
            {adminTab==="posts"&&(<>
              <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:10}}>중고 물건 ({items.length})</div>
              {items.map(item=>(<div key={item.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                <div style={{width:36,height:36,borderRadius:8,background:LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{item.emoji||"📦"}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:500,marginBottom:1}}>{item.title}</div>
                  <div style={{fontSize:11,color:"#aaa"}}>{item.seller} · {item.region}</div>
                  <div style={{display:"flex",gap:4,marginTop:2}}>{itemBadges(item).map((b,i)=><span key={i}>{b}</span>)}</div>
                </div>
                <button onClick={()=>deleteItem(item.id)} style={{background:"#ffebee",border:"none",borderRadius:8,padding:"5px 8px",fontSize:11,color:"#c62828",cursor:"pointer",flexShrink:0}}>삭제</button>
              </div>))}
              <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginTop:16,marginBottom:10}}>일자리 공고 ({jobs.length})</div>
              {jobs.map(job=>(<div key={job.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                <div style={{width:36,height:36,borderRadius:8,background:LIGHT,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{job.icon}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:500,marginBottom:1}}>{job.title}</div>
                  <div style={{fontSize:11,color:"#aaa"}}>{job.org} · {job.location}</div>
                  <div style={{display:"flex",gap:4,marginTop:2}}>{jobBadges(job).map((b,i)=><span key={i}>{b}</span>)}</div>
                </div>
                <button onClick={()=>deleteJob(job.id)} style={{background:"#ffebee",border:"none",borderRadius:8,padding:"5px 8px",fontSize:11,color:"#c62828",cursor:"pointer",flexShrink:0}}>삭제</button>
              </div>))}
            </>)}
            ...
            {adminTab==="stats"&&(<>
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:12}}>📦 카테고리별 게시글</div>
                <BarChart data={catStats} color={ACCENT}/>
              </div>
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:12}}>📍 지역별 분포</div>
                <BarChart data={regionStats} color={ADMIN}/>
              </div>
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:12}}>🏷 나누미 / 구하미 비율</div>
                {(()=>{const nan=items.filter(i=>i.postType==="nanumi").length;const gu=items.filter(i=>i.postType==="guhami").length;const tot=nan+gu||1;return(<div><div style={{display:"flex",height:20,borderRadius:10,overflow:"hidden",marginBottom:8}}><div style={{width:`${(nan/tot)*100}%`,background:ACCENT}}/><div style={{width:`${(gu/tot)*100}%`,background:"#c62828"}}/></div><div style={{display:"flex",gap:16,fontSize:12}}><span style={{color:ACCENT}}>■ 나누미 {nan}개 ({Math.round((nan/tot)*100)}%)</span><span style={{color:"#c62828"}}>■ 구하미 {gu}개 ({Math.round((gu/tot)*100)}%)</span></div></div>);})()}
              </div>
              <div>
                <div style={{fontSize:12,color:"#aaa",fontWeight:500,marginBottom:12}}>👥 계정 유형</div>
                {(()=>{const dan=users.filter(u=>u.accountType==="단체").length;const gae=users.filter(u=>u.accountType==="개인"||u.accountType==="관리자").length;const tot=users.length||1;return(<div><div style={{display:"flex",height:20,borderRadius:10,overflow:"hidden",marginBottom:8}}><div style={{width:`${(dan/tot)*100}%`,background:ADMIN}}/><div style={{flex:1,background:"#e0e0e0"}}/></div><div style={{display:"flex",gap:16,fontSize:12}}><span style={{color:ADMIN}}>■ 단체 {dan}명</span><span style={{color:"#888"}}>■ 개인 {gae}명</span></div></div>);})()}
              </div>
            </>)}

            {/* 신고 관리 */}
            {adminTab==="reports"&&(<>
              <div style={{fontSize:12,color:"#aaa",marginBottom:12}}>신고 접수 목록</div>
              {reports.map(r=>(<div key={r.id} style={{padding:"14px 0",borderBottom:"0.5px solid #f5f5f5"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                  <div>
                    <span style={{fontSize:10,padding:"2px 8px",borderRadius:10,background:r.status==="검토중"?"#ffebee":"#f5f5f5",color:r.status==="검토중"?"#c62828":"#9e9e9e",fontWeight:500,marginRight:6}}>{r.status}</span>
                    <span style={{fontSize:11,color:"#aaa"}}>{r.type} 신고</span>
                  </div>
                  <span style={{fontSize:11,color:"#bbb"}}>{r.date}</span>
                </div>
                <div style={{fontSize:13,fontWeight:500,marginBottom:2}}>"{r.target}"</div>
                <div style={{fontSize:12,color:"#888",marginBottom:8}}>신고 사유: {r.reason}</div>
                {r.status==="검토중"&&<div style={{display:"flex",gap:8}}><button onClick={()=>setReports(p=>p.map(rr=>rr.id===r.id?{...rr,status:"처리완료"}:rr))} style={{flex:1,padding:"7px 0",borderRadius:10,border:"none",background:"#e8f5e9",color:"#2e7d32",fontSize:12,cursor:"pointer",fontWeight:500}}>처리 완료</button><button onClick={()=>setReports(p=>p.map(rr=>rr.id===r.id?{...rr,status:"반려"}:rr))} style={{flex:1,padding:"7px 0",borderRadius:10,border:"none",background:"#f5f5f5",color:"#888",fontSize:12,cursor:"pointer"}}>반려</button></div>}
              </div>))}
            </>)}
          </div>
        </div>
      ...
      {showAdminLogin&&(
        <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
          <div style={{background:"#fff",borderRadius:20,padding:"28px 24px",width:280,boxSizing:"border-box"}}>
            <div style={{fontSize:16,fontWeight:600,color:ADMIN,marginBottom:4}}>🔐 관리자 로그인</div>
            <div style={{fontSize:12,color:"#aaa",marginBottom:16}}>비밀번호를 입력하세요</div>
            <input value={adminPw} onChange={e=>setAdminPw(e.target.value)} type="password" placeholder="비밀번호" onKeyDown={e=>{if(e.key==="Enter"){if(adminPw===ADMIN_PW){setIsAdmin(true);setScreen("admin");setShowAdminLogin(false);setAdminPw("");}else{alert("비밀번호가 틀렸습니다.");}}} } style={{...inp,marginBottom:12}}/>
            <button onClick={()=>{if(adminPw===ADMIN_PW){setIsAdmin(true);setScreen("admin");setShowAdminLogin(false);setAdminPw("");}else{alert("비밀번호가 틀렸습니다.");}}} style={{width:"100%",height:44,borderRadius:12,border:"none",background:ADMIN,color:"#fff",fontSize:14,fontWeight:500,cursor:"pointer",marginBottom:8}}>확인</button>
            <button onClick={()=>{setShowAdminLogin(false);setAdminPw("");}} style={{width:"100%",background:"none",border:"none",color:"#aaa",fontSize:13,cursor:"pointer",padding:"8px 0"}}>취소</button>
          </div>
        </div>
      )}
